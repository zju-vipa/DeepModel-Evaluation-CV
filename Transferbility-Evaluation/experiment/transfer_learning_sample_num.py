...
from dataset import tinyfood101, tinycaltech, tinydomainnet, tinydtd
...
parser.add_argument('--checkpoint_save_step', type=int, default=1)
parser.add_argument('--log_path', type=str)
parser.add_argument('--end_epoch', type=str, default=5)
...
def transfer_learning(args):
    ...
    sample_nums = [1, 5, 10 ,20 ,50 ,80]
    for i, sample_num in enumerate(sample_nums):
        torch.cuda.empty_cache()
        #迁移学习checkpoint设置
        checkpoint_path = os.path.join(args.checkpoint_path, f'imagenet_{sample_num}{args.target_dataset}', settings.TIME_NOW)
        if not os.path.exists(checkpoint_path):
            os.makedirs(checkpoint_path)
        checkpoint_path = os.path.join(checkpoint_path, '{arch}-{epoch}-{type}.pth')
        #获取数据集和dataloader
        train_loader, test_loader = get_dataset_and_loader(args, class_num=45, sample_num=sample_num)
        start_epoch = 1
        num_classes = NUM_CLASSES[args.target_dataset]
        #获取预训练模型
        ...
        # 余弦退火学习率
        train_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=600)
        # StepLR
        ...
        for epoch in range(start_epoch, args.end_epoch+1):
            train(epoch, model, train_loader, optimizer, loss_function, writer, sample_num, args.arch)
            acc, test_loss = eval_training(epoch, model, test_loader, loss_function, writer)

            logging.info('Sample_num: {:d} Epoch {:d} Test set: LR: {:.8f},  Average loss: {:.4f}, Accuracy: {:.4f}'.format(
                sample_num, epoch, optimizer.param_groups[0]['lr'], test_loss, acc 
            ))
            
            if epoch > settings.TR_TOTAL_EPOCH / 2 and best_acc < acc:
        ...
        writer.add_scalar(f'Acc/Sample_num', best_acc, i)
        
    writer.close()


def train(epoch, model, train_loader, optimizer, loss_function, writer, sample_num,  model_name=None):
    ...
    for batch_index, (images, labels) in enumerate(tqdm_train_loader):
        ...
        tqdm_train_loader.set_description('Sample numTraining Epoch: {epoch}, Loss: {:0.4f}, LR: {:0.6f}'.format(
            loss.item(),
        ...
        writer.add_scalar(f'{sample_num}Train/loss', loss.item(), n_iter)
